...
states :: LTS -> [State]
states a = nub (0 : [b | ((_, b),_) <- a])

transitions :: State -> LTS -> [Transition]
transitions x b = [c | (c@((a,_),_)) <- b, a == x]
...
actions STOP = []
actions (Ref _) = []
actions (Prefix a b) = a : actions b
actions (Choice a) = concatMap actions a

accepts :: [Id] -> [ProcessDef] -> Bool
--Pre: The first item in the list of process definitions is
--     that of the start process.
accepts i pd@(p:ps) = accepts' i p
   where
     accepts' [] _ = True
     accepts' _ (_, STOP) = False
     accepts' a (i, Ref x)
        = accepts' a (i, (lookUp x pd))
     accepts' (a:as) (i, Prefix b c)
        | a == b = accepts' as (i, c)
        | otherwise = False
     accepts' a (i, Choice b)
        = or (map (\p -> accepts' a (i, p)) b)
