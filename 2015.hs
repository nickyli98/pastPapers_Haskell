...
getLocals a = [x | x@(_, (y, _)) <- a, isLocal y]

getGlobals :: State -> State
getGlobals a = [x | x@(_, (y, _)) <- a, not (isLocal y)]
...
assignArray (A a) (I i) (I v) = A (map findA a)
   where
     findA (a, b)
        | a == i = (a, v)
        | otherwise = (a, b)
...
updateVar _ [] = []
updateVar (i, v) state@((a, b):ss)
   | notExist i state = ((i, (Global, v)) : state)
   | i == a = (i, (Local, v)) : ss
   | otherwise = (a, b) : updateVar (i, v) ss

notExist _ [] = True
notExist i ((a, b) : ss)
  | a == i = False
  | otherwise = notExist i ss
---------------------------------------------------------------------
-- Part II
...
applyOp Equal (I a) (I b)
   | a == b = I 1
   | otherwise = I 0
applyOp Index (A a) (I b)
   | notExist b a = I 0
   | otherwise = I (lookUp b a)


bindArgs :: [Id] -> [Value] -> State
-- Pre: the lists have the same length
bindArgs [] [] = []
bindArgs (a:as) (b:bs) = (a, (Local, b)) : bindArgs as bs

evalArgs :: [Exp] -> [FunDef] -> State -> [Value]
evalArgs a b c = map evalAll a
   where
     evalAll a = eval a b c
...
eval (Const c) _ _ = c
eval (Var a) _ b = getValue a b
eval (Cond a b c) x s
   | (eval a x s) == I 1 = eval b x s
   | otherwise = eval c x s
eval (OpApp a b c) x s
   = applyOp a (eval b x s) (eval c x s)
eval (FunApp a b) x s
   = eval e x (state ++ s)
     where
      (as, e) = lookUp a x
