...
tryToLookUp a b c = maybe b id (lookup a c)

-- Pre: The given value is in the table
reverseLookUp :: Eq b => b -> [(a, b)] -> [a]
reverseLookUp a b = [x | (x, y) <- b, y == a]

occurs :: String -> Type -> Bool
occurs x (TVar a) = x == a
occurs x (TFun t t') = occurs x t || occurs x t'
occurs x _ = False
...
inferType (Number x) _ = TInt
inferType (Boolean x) _ = TBool
inferType (Id x) b = lookUp x b
inferType (Prim x) _ = lookUp x primTypes
inferType (Cond (Boolean x) b c) z
   = inferType' b c z
   where
     inferType' a b x
        | inferType a x /= inferType b x = TErr
        | otherwise = inferType a x
inferType (Cond _ _ _) _ = TErr
inferType app@(App a b) c
   = inferApp app c

inferApp :: Expr -> TEnv -> Type
inferApp (App a b) c
  | inferType a c == TErr = TErr
  | inferType b c == t1 = t2
  | otherwise = TErr
  where
    (TFun t1 t2) = inferApp a c

------------------------------------------------------
-- PART III
applySub :: Sub -> Type -> Type
applySub _ (TInt _) = TInt
applySub _ (TBool _) = TBool
applySub s (Id x) = tryToLookUp x s
applySub s (TFun a b) = TFun (applySub s a) (applySub s b)
applySub s (Cond a b c) = Cond (applySub s a) (applySub s b) (applySub s c)
applySub s (App a b) = App (applySub s a) (applySub s b)
